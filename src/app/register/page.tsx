        ...
        if (uName && lName && email && password && cPassword && phone) {
            if (password === cPassword) {
                if (phone.length === 10) {
                    ...
                    registerFunction(uName, lName, email, password, phonePref)
                        .then((res) => {
                            if (res?.email != null) {
            ...
            <div className="font-[sans-serif] p-8 mx-auto max-w-lg text-center">
                ...
                <form onSubmit={(e) => e.preventDefault()}>
                    <div className="grid grid-cols-1 gap-5">
                        {[
                            { label: "First Name", value: uName, onChange: setName, placeholder: "Enter first name" },
                    ...
                    <div className="mt-8 flex flex-col space-y-3 items-center">
                        <button 
                            type="button" 
                            onClick={onSubmit}
                            className="w-full py-3 px-6 text-sm font-semibold tracking-wider rounded-md text-white bg-[#4CAF50] hover:bg-green-600 transition-all"
                        >
                            Sign Up
                        </button>
                        <button 
